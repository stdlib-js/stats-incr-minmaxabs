...
tape( 'the function returns an accumulator function', function test( t ) {
	t.strictEqual( typeof incrminmaxabs(), 'function', 'returns expected value' );
	t.end();
});

tape( 'the function returns an accumulator function (output)', function test( t ) {
	t.strictEqual( typeof incrminmaxabs( [ 0.0, 0.0 ] ), 'function', 'returns expected value' );
	t.end();
});
...
tape( 'the accumulator function computes a minimum and maximum absolute value incrementally (output)', function test( t ) {
	...
	for ( i = 0; i < N; i++ ) {
		...
		t.strictEqual( actual[ i ], out, 'returns output array' );
